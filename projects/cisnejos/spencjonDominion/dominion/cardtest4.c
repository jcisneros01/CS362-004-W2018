...
int main() {
	...
	printf("TEST 1: hand +1 card\n");
	printf("hand count = %d, expected = %d\n", State.handCount[0], StateCopy.handCount[0]);
	assertEqual(StateCopy.handCount[0], State.handCount[0]);

	// cards should come from his own pile.
	printf("TEST 2: deck -1 cards\n");
	printf("deck count = %d, expected = %d\n", State.deckCount[0], StateCopy.deckCount[0] - 1);
	assertEqual(StateCopy.deckCount[0] - 1, State.deckCount[0]);

	// Current player should receive exactly 2 actions
	printf("TEST 3: actions +2\n");
	printf("deck count = %d, expected = %d\n", State.numActions, StateCopy.numActions + 2);
	assertEqual(StateCopy.numActions + 2, State.numActions);

	// No state change should occur for other players.
	printf("TEST 4: No change to player's Deck or Hand\n");
